...
# Convert 'Date' to datetime objects immediately for easier filtering later
...
# Use .sum() on the Units column, skipping NaNs by default
...
# Use .sum() on the Total_Sales column
...
# Note: Unit_Price has missing values (NaN) only where Units are missing, but Total_Sales is present.
# We'll calculate the average of the existing Unit_Price values.
...
# Use the & operator for combining multiple boolean conditions
...
# Since 'Date' is datetime, we can use dt.month
...
# Group by 'Product', sum 'Total_Sales', find the index of the maximum value
...
# Group by 'Region', sum 'Total_Sales', and sort
...
# Group by 'Category', calculate mean of 'Units'
...
# Note on Missing Data Implications:
# 'Units' and 'Unit_Price' are missing in 2 and 3 rows, respectively.
# The missing 'Unit_Price' and 'Units' will lead to undercounting of total units and
# distortion in the average unit price calculation if not addressed. However, since
# 'Total_Sales' is present for all but one of these missing Unit/Price records,
# the Total Revenue calculation (2.3) is largely unaffected (only one total revenue is implicitly missing,
# where both Units and Unit_Price are missing: the Latin America Accessories sale on 2024-03-25 has Total_Sales=999.60
# but Units is missing. The other two missing values for Units/Price have a Total_Sales.
# Let's re-examine the data:
# 2024-02-25,Latin America,LA002,Smartphones,PhoneX,9,,8099.91,No -> Unit_Price is missing
# 2024-03-02,North America,NA003,Wearables,FitnessTracker,,129.99,2599.80,Yes -> Units is missing
# 2024-03-25,Latin America,LA002,Accessories,PhoneCase,40,,999.60,No -> Unit_Price is missing
# There are 3 missing Unit_Price values, and 2 missing Units values.
# The Total_Sales column itself is clean (0 missing). The implication is that we can't reliably
# calculate Unit Price for the two records where it is missing, and we undercount total units.

print("-" * 60)

# TODO 6: Insights and Business Analysis
# 6.1 Create a summary of the top-performing category in each region
# Group by Region and Category, sum Total_Sales, and then use idxmax() at the Region level
...
# Calculate Average Sale Value for the report
...
print("Regional Performance (Total Sales):")
...
# Combine average units and average price series
...
no_promo_total_revenue = promo_comparison['no_promo_total_revenue']
...
print(f"\nInsight: Promoted items have a higher average sale value (${promo_avg_sales:,.2f} vs ${no_promo_avg_sales:,.2f}), indicating promotions are effectively used on higher-priced items.")
print("-" * 30)
...
if total_missing_entries > 0:
    ...
    print(f"Note: Missing 'Units' (in 2 records) and 'Unit_Price' (in 3 records) will slightly distort Total Units and Average Unit Price metrics.")
else:
...
print("2. Product Strategy: Leverage 'LaptopPro' (highest revenue product) and 'Smartphones' (top category in multiple regions) through focused marketing campaigns. Consider bundling high-volume, low-price 'Accessories' to increase basket value.")
