...
print(f"\nInitial Duplicate Count (rows that are duplicated): {initial_duplicate_count}")
...
date_fill_strategy = 'drop_rows_with_unrecoverable_last_purchase'
...
duplicate_count = df_text_cleaned.duplicated(subset=['customer_id'], keep=False).sum()
...
print("## Data Quality Issues Addressed 🛠️")
...
print(f"- **Customer Base:** {len(df_final)} total customers (after cleaning).")
...
print(f"- **Satisfaction vs. Spend:** Correlation between `Satisfaction_Rating` and `Total_Spent` is **{satisfaction_spend_corr}**. (A weak positive relationship, suggesting satisfied customers spend slightly more.)")
print("\n" + "-" * 60)
